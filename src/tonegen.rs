...
impl ToneGenerator {
    ...
    pub fn new(
        ...
        if pool.is_null() {
            return Err(PjError::Pjsip {
                status: -1,
                message: "Failed to create pool for ToneGenerator".into(),
            });
        }
        ...
        debug!(
            "tone generator created: rate={clock_rate} ch={channel_count} spf={samples_per_frame}"
        );
        Ok(ToneGenerator {
            port,
            pool,
    ...
    pub fn play_digits(&self, digits: &[DtmfDigit]) -> Result<()> {
        ...
        let c_digits: Vec<ffi::pjmedia_tone_digit> = digits
            .iter()
            .map(|d| ffi::pjmedia_tone_digit {
                digit: d.digit as _,
                on_msec: d.on_ms,
                off_msec: d.off_ms,
                volume: d.volume,
            ...
            .collect();
        let status = unsafe {
            ffi::pjmedia_tonegen_play_digits(self.port, c_digits.len() as u32, c_digits.as_ptr(), 0)
        };
        check_status(status)
    }
    ...
    pub fn play_tones(&self, tones: &[ToneDesc]) -> Result<()> {
        ...
        let c_tones: Vec<ffi::pjmedia_tone_desc> = tones
            .iter()
            .map(|t| ffi::pjmedia_tone_desc {
                freq1: t.freq1,
                freq2: t.freq2,
                on_msec: t.on_ms,
                off_msec: t.off_ms,
            ...
            .collect();
        let status = unsafe {
            ffi::pjmedia_tonegen_play(self.port, c_tones.len() as u32, c_tones.as_ptr(), 0)
        };
        check_status(status)
    }
